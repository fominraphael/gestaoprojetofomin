...
// Mescla checklist + laudo cautelar + health check de um veículo e salva em
// storage atualizando dossie_pdf_path. Chamada em fire-and-forget a partir do
// frontend (Pós-Vendas → Análise Central e regeração na tela de elegíveis).
...
async function processar(veiculo_id: string) {
  ...
  const merged = await mesclar(pdfs);
