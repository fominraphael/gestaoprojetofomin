...
function chamadoToEdit(c: Chamado): EditForm {
  return {
    ...
    loja_estoque: c.loja_estoque ?? "",
    codigo_avaliacao_nbs: c.codigo_avaliacao_nbs ?? "",
...
function DetalheChamado() {
  ...
  const [dialogo, setDialogo] = useState<
    null | "pendenciar" | "resolver" | "comprar" | "cancelar" | "suspender" | "reativar"
  >(null);
  ...
  const [debitoPend, setDebitoPend] = useState<null | {
    tipo: string;
    label: string;
    statusValor: string;
    statusLabel: string;
  ...
  useEffect(() => {
    (async () => {
      const [lojasRes, stRes, typesRes] = await Promise.all([
        supabase
        ...
        supabase
          .from("compras_cadastros")
          .select("valor,label,grupo,exige_anexo,exige_descricao")
          .eq("categoria", "status_debito")
          .eq("ativo", true)
          .order("ordem"),
  ...
  const carregar = useCallback(async () => {
    ...
    if (ids.length) {
      ...
      (profs ?? []).forEach((p: any) => {
        map[p.id] = p.nome_fantasia || p.username;
      });
      setAutores(map);
    } else setAutores({});
    setLoading(false);
  ...
  // Usuários não-admin entram direto em modo assumido
  useEffect(() => {
    if (!chamado || isAdmin || askedRef.current) return;
    const finalizado = chamado.status === "comprado" || chamado.status === "cancelado";
    if (finalizado) return;
  ...
  async function assumir() {
    ...
    const novoStatus: StatusChamado =
      chamado.status === "na_fila_central" ? "em_analise" : chamado.status;
  ...
  async function enviarPending() {
    ...
    try {
      for (const p of pending) {
        ...
        if (upErr) {
          fail++;
          toast.error(`${p.file.name}: ${upErr.message}`);
          continue;
        }
        ...
        if (insErr) {
          fail++;
          toast.error(`${p.file.name}: ${insErr.message}`);
          continue;
        }
        await registrarHistorico({
          acao: "documento_anexado",
          observacao: p.file.name,
          campo: p.categoria,
        });
        ok++;
      }
      setPending([]);
      if (ok > 0)
        toast.success(`${ok} documento(s) anexado(s)${fail ? ` — ${fail} falha(s)` : ""}.`);
  ...
  async function marcarDebito(tipo: string, statusValor: string) {
    ...
    if (opt.exige_anexo || opt.exige_descricao) {
      ...
      setDebitoPend({
        tipo,
        label: tipoLabel,
        statusValor: opt.valor,
        statusLabel: opt.label,
        exigeAnexo: opt.exige_anexo,
        exigeDescricao: opt.exige_descricao,
      });
      return;
    }
    ...
    setDebitos((prev) => {
      ...
      if (idx >= 0) {
        const copy = [...prev];
        copy[idx] = { ...copy[idx], status: statusValor };
        return copy;
      }
    ...
    await registrarHistorico({
      acao: "debito_marcado",
      campo: tipo,
      valor_antes: anterior,
      valor_depois: statusValor,
    });
  }

  async function confirmarDebitoPendente() {
    if (!chamado || !debitoPend) return;
    if (debitoPend.exigeAnexo && !debFile) {
      toast.error("Anexo obrigatório para este status.");
      return;
    }
    if (debitoPend.exigeDescricao && !debObs.trim()) {
      toast.error("Descrição obrigatória para este status.");
      return;
    }
    setDebSaving(true);
    try {
      ...
      const payload: any = {
        chamado_id: chamado.id,
        tipo: debitoPend.tipo,
        status: debitoPend.statusValor,
        observacao: debObs.trim() || null,
      };
      if (path) payload.comprovante_path = path;
      const { error } = await supabase
        .from("compras_debitos")
        .upsert(payload, { onConflict: "chamado_id,tipo" });
      if (error) {
        toast.error(error.message);
        return;
      }
      setDebitos((prev) => {
        ...
        const item: Debito = {
          tipo: debitoPend.tipo,
          status: debitoPend.statusValor,
          comprovante_path: path ?? undefined,
          observacao: debObs.trim() || undefined,
        };
        if (idx >= 0) {
          const copy = [...prev];
      ...
      await registrarHistorico({
        acao: "debito_status_alterado",
        campo: debitoPend.tipo,
        valor_antes: anterior,
        valor_depois: debitoPend.statusValor,
        observacao: debObs.trim() || null,
        anexo_path: path,
      });
  ...
  async function enviarParaFila() {
    ...
    if (docsFalt.length || debFalt.length) {
      ...
      if (debFalt.length)
        parts.push(`Itens de checagem/débitos obrigatórios pendentes: ${debFalt.join(", ")}`);
  ...
  async function confirmarAcao() {
    ...
    if (dialogo === "comprar" && !observ.trim()) {
      toast.error("Informe a observação da compra.");
      return;
    }
      ...
      try {
        await notificar({
          data: {
            ...
            tipo: dialogo === "pendenciar" ? "pendenciado" : "resolvido",
            motivo,
            observacao: observ,
          },
        });
  ...
  async function salvarEdicaoAdmin() {
    ...
    try {
      ...
      if ((editForm.loja_estoque ?? "") !== (atual.loja_estoque ?? "")) {
        ...
        mudancas.push({
          campo: "loja_estoque",
          label: "Loja de estoque",
          antes: atual.loja_estoque,
          depois: editForm.loja_estoque,
        });
      }
      // Status (select)
  ...
  const podeAdminSuspensao =
    isAdmin && !!perfilSuspensao && user?.tipo_usuario === perfilSuspensao.nome;
  const STATUS_EDITAVEIS_CRIADOR: StatusChamado[] = [
    "documentacao",
    "na_fila_central",
    "pendenciado",
  ];
  ...
  const podePendenciar =
    ...
    (chamado.status === "em_analise" ||
      chamado.status === "na_fila_central" ||
      chamado.status === "documentacao");
  const podeResolver =
    (isCriador || (isAdmin && !readOnlyAdmin)) && chamado.status === "pendenciado";
  const podeComprar =
    podeAgirCentral && (chamado.status === "em_analise" || chamado.status === "na_fila_central");
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold">
              {chamado.placa} — {chamado.nome}
            </h1>
            <p className="text-sm text-muted-foreground">
              {chamado.tipo_pessoa} • {chamado.estado_uf} •{" "}
              {TIPO_COMPRA_LABEL[chamado.tipo_compra as keyof typeof TIPO_COMPRA_LABEL]}
            </p>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            <div>
              <strong>Chassi:</strong> {chamado.chassi ?? "-"}
            </div>
            <div>
              <strong>Modelo:</strong> {chamado.modelo ?? "-"} {chamado.ano_modelo ?? ""}
            </div>
            <div>
              <strong>Loja estoque:</strong> {chamado.loja_estoque ?? "-"}
            </div>
            <div>
              <strong>Cód. avaliação NBS:</strong> {chamado.codigo_avaliacao_nbs ?? "-"}
            </div>
            <div>
              <strong>Valor avaliado:</strong>{" "}
              {chamado.valor_avaliado != null
                ? `R$ ${Number(chamado.valor_avaliado).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`
                : "-"}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            <div>
              <strong>Nome/Razão:</strong> {chamado.nome}
            </div>
            <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Documentos ({chamado.estado_uf} • {chamado.tipo_pessoa})
          </CardTitle>
        </CardHeader>
              ...
              {pending.map((p, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setPending((prev) =>
                        prev.map((x, j) => (j === i ? { ...x, categoria: v } : x)),
                      )
                    }
                  >
                      ...
                      {enviados.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {enviados.map((d) => (
                            <div
                              key={d.id}
                              className="inline-flex items-center gap-1 rounded bg-muted/40 px-1.5 py-0.5 text-xs"
                            >
                              <button
                                onClick={() => abrirDoc(d.storage_path)}
                                className="hover:underline inline-flex items-center gap-1"
                              >
                                <Eye className="w-3 h-3" /> ver
                              </button>
                              <button
                                onClick={() => baixarDoc(d.storage_path)}
                                className="hover:underline inline-flex items-center gap-1 text-foreground/80"
                                title="Baixar"
                              >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.files) {
                                ...
                                setPending((prev) => [
                                  ...prev,
                                  ...arr.map((f) => ({ file: f, categoria: req.categoria })),
                                ]);
                              }
                              e.target.value = "";
                            }}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Débitos / itens de checagem</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {TIPOS_DEBITO.map((t) => {
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
          {podeEnviarFila && <Button onClick={enviarParaFila}>Enviar para a fila (Central)</Button>}
          {podePendenciar && (
            ...
            <ul className="space-y-2 text-sm">
              {historico.map((h) => (
                <li key={h.id} className="border border-border rounded-md p-2">
                  ...
                  {h.observacao && (
                    <div className="text-xs text-muted-foreground mt-1">{h.observacao}</div>
                  )}
                </li>
              ))}
            </ul>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Alterações ficam registradas no log com valor anterior e novo.
            </DialogDescription>
          </DialogHeader>
          {editForm && (
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setEditForm((f) => (f ? { ...f, status: v as StatusChamado } : f))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
      ...
      <Dialog open={!!debitoPend} onOpenChange={(o) => !o && setDebitoPend(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {debitoPend?.label} → {debitoPend?.statusLabel}
            </DialogTitle>
            <DialogDescription>
              {[
                debitoPend?.exigeDescricao ? "descrição" : null,
                debitoPend?.exigeAnexo ? "anexo" : null,
              ]
                .filter(Boolean)
                .join(" e ")}{" "}
              obrigatório(s) para este status.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setDebitoPend(null)}>
              Cancelar
            </Button>
      ...
      <Dialog open={askAdmin} onOpenChange={setAskAdmin}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você pode apenas visualizar sem alterar nada, ou assumir o processo para tomar ações
              da Central.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!dialogo} onOpenChange={(o) => !o && setDialogo(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            {dialogo === "suspender" && (
              <DialogDescription>
                O chamado ficará suspenso até que terceiros resolvam a pendência. Apenas você poderá
                reativá-lo.
              </DialogDescription>
            )}
            {dialogo === "reativar" && (
            ...
            {dialogo !== "reativar" && (
              <div>
                ...
                <Textarea
                  ...
                  placeholder={
                    dialogo === "comprar"
                      ? "Obrigatório: descreva condições, valores acordados, etc."
                      : ""
                  }
                />
              </div>
