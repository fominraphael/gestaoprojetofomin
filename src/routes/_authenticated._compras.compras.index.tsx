...
function ComprasIndex() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("compras-colunas-v2", JSON.stringify(colunasVisiveis));
    } catch {
  ...
  async function carregar() {
    ...
    const { data } = await supabase
      ...
      .select(
        "id, placa, nome, tipo_pessoa, tipo_compra, estado_uf, status, created_at, valor_avaliado, loja_estoque, criado_por, assumido_por, ordem",
      )
    ...
    const userIds = Array.from(
      new Set(list.flatMap((r) => [r.criado_por, r.assumido_por]).filter(Boolean)),
    ) as string[];
    ...
    (prof ?? []).forEach((p: any) => {
      um[p.id] = p.nome_fantasia || p.username;
    });
    setUsuarios(um);
  ...
  const kpis = useMemo(() => {
    ...
    const emFluxo = rowsPorPeriodo.filter((r) =>
      ["documentacao", "na_fila_central", "em_analise"].includes(r.status),
    ).length;
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      case "criado_asc":
        ...
        break;
      case "placa":
        sorted.sort((a, b) => (a.placa ?? "").localeCompare(b.placa ?? ""));
        break;
      case "nome":
        sorted.sort((a, b) => (a.nome ?? "").localeCompare(b.nome ?? ""));
        break;
      case "valor_desc":
        sorted.sort((a, b) => (Number(b.valor_avaliado) || 0) - (Number(a.valor_avaliado) || 0));
        break;
      case "valor_asc":
        sorted.sort((a, b) => (Number(a.valor_avaliado) || 0) - (Number(b.valor_avaliado) || 0));
        break;
      case "status":
  ...
  async function mover(id: string, dir: -1 | 1) {
    ...
    try {
      const list = [...filtered];
      list.forEach((r, i) => {
        if (r.ordem == null) r.ordem = (i + 1) * 10;
      });
    ...
    <div className="p-6 space-y-4 max-w-none">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => navigate({ to: "/compras/novo" })}
            title="Novo chamado"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <KpiCard
          icon={AlertTriangle}
          label="Pendenciados"
          valor={kpis.pendenciados}
          tint="text-amber-500"
          onClick={() => setStatus("pendenciado")}
          ativo={status === "pendenciado"}
        />
        <KpiCard
          icon={CheckCircle2}
          label="Comprados"
          valor={kpis.comprados}
        ...
        <KpiCard
          ...
          valor={`R$ ${kpis.valorComprado.toLocaleString("pt-BR", { minimumFractionDigits: 0 })}`}
          tint="text-emerald-500"
        />
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Select value={sortBy} onValueChange={(v) => setSortBy(v as typeof sortBy)}>
          <SelectTrigger className="w-56" title="Ordenar por">
      ...
      <AlertDialog open={!!excluir} onOpenChange={(o) => !o && setExcluir(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              O chamado <b>{excluir?.placa}</b> ({excluir?.nome}) e todos os seus documentos,
              débitos e histórico serão removidos permanentemente. Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function renderCell(r: ChamadoRow, col: ColId, ctx: CellCtx) {
  switch (col) {
    ...
    case "nome":
      return r.nome;
    case "solicitante":
      return r.criado_por ? (ctx.usuarios[r.criado_por] ?? "—") : "—";
    case "central":
      return r.assumido_por ? (
        (ctx.usuarios[r.assumido_por] ?? "—")
      ) : (
        <span className="text-muted-foreground text-xs">—</span>
      );
    case "loja":
      return r.loja_estoque ? (ctx.lojasMap[r.loja_estoque] ?? r.loja_estoque) : "—";
    case "tipo_pessoa":
      return r.tipo_pessoa;
    case "estado_uf":
      return r.estado_uf;
    case "tipo_compra":
      return TIPO_COMPRA_LABEL[r.tipo_compra as keyof typeof TIPO_COMPRA_LABEL] ?? r.tipo_compra;
    case "valor":
      return r.valor_avaliado != null
        ? `R$ ${Number(r.valor_avaliado).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`
        : "-";
    case "status":
      return (
    ...
    case "criado_em":
      return new Date(r.created_at).toLocaleDateString("pt-BR");
    default:
      return null;
  }
}
...
function KpiCard({
  ...
  return (
    ...
    >
      <div
        className={`w-9 h-9 rounded-md bg-muted flex items-center justify-center ${tint ?? "text-muted-foreground"}`}
      >
